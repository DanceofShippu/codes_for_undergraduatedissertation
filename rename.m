clear
folderName = 'E:\datatranfer';
% AA = dir(fullfile(folderName,'*.zip'));
%AA = dir(fullfile(folderName,'*.jpg'));
AA = dir(fullfile(folderName,'*.tif'));


AA= {AA.name};
for i = 1:length(AA)    
    NameTemp = char(AA(i));
    [a,b] = ismember(NameTemp,'_');
    [c,d] = ismember(NameTemp,'mm');
    Loc = find(a);
    Loc2 = find(c);
    XLoc =  str2double(NameTemp(Loc2(1)-8:Loc2(1)-2));
%     name2{i} = NameTemp(Loc(end-4):end);
    name2{i} = strcat(NameTemp(Loc(1):Loc2(1)-9),'_',num2str(XLoc),'_',NameTemp(Loc2(1):end));
end
[aa,bb] = natsort(name2);
for i = 1:length(AA)
    Name = aa(i);
    Name = strcat('000', Name);
    cc = length(num2str(i));
    Name = char(Name);
    Name((4-cc):3) = num2str(i);
    movefile(fullfile(folderName,char(AA(bb(i)))),fullfile(folderName,Name)); 
end
...
% for i = 1:length(BB) 
%     Name = BB(i);
%     Name = strcat('000_', Name);
%     cc = length(num2str(i));
%     Name = char(Name);
%     Name((4-cc):3) = num2str(i);
%     movefile(fullfile(folderName,char(BB(i))),fullfile(folderName,Name)); 
% end
